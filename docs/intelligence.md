...
Version 0.9 separates intelligence from evidence collection. Discovery and validation remain
...
described records. Markup is removed first, titles are weighted, and each outlier includes its title.
Results are deterministic for a fixed input and do not modify the catalogue.
...
Providers are isolated: a failure from one provider does not discard successful classifications
from the other. The report is checkpointed after every attempt and includes structured errors, so
invalid models, quotas and schema failures remain auditable without losing partial output.
